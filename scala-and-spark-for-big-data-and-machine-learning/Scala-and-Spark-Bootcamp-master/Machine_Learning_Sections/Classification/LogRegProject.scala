...
val dataWithTime = data.withColumn("Hour", hour(data("Timestamp")))
val logregdata = (dataWithTime.select(
...
val logRModel = pipeline.fit(training)

// Get Results on Test Set with transform
val result = logRModel.transform(test)
